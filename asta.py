...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.استا"))
async def _(event):
        await event.edit("شغال ميه ميه\nasta @ddssss , @astashope")

@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.المطور"))
async def _(event):
        await event.edit("**- Dady: @ddssss \ @astashope**")

@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.الاوامر"))
async def _(event):
        await event.edit("`.بوت المليار`")
